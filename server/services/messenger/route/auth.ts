...
export default ({ rabbitMQChannel }: InitRouterParams): Router => {
    ...
    router.post("/", validate(authSchema), async (req: Request, res: Response) => {
        try {
            ...
            */
            if (+process.env.ALLOW_MULTIPLE_MOBILE_APP_DEVICES) {
            ...
            console.log("aaa", process.env.IS_TEST, isTester(telephoneNumber));

            if (process.env.IS_TEST !== "1" && !isTester(telephoneNumber)) {
