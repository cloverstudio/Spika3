...
class Room extends EventEmitter {
     ...
     */
    handleProtooConnection(param) {
        const { peerId, consume, protooWebSocketTransport } = param;
        ...
        try {
            ...
            le("protooRoom.createPeer() failed:%o", error);
        }
        ...
        peer.on("request", (request, accept, reject) => {
            l('protoo Peer "request" event [method:%s, peerId:%s]', request.method, peer.id);

            this._handleProtooRequest(peer, request, accept, reject).catch((error) => {
                le("request failed:%o", error);
        ...
        peer.on("close", () => {
            ...
            l('protoo Peer "close" event [peerId:%s]', peer.id);
            ...
            if (this._protooRoom.peers.length === 0) {
                l("last Peer in the room left, closing the room [roomId:%s]", this._roomId);
        ...
        switch (request.method) {
            ...
            case "createWebRtcTransport": {
                ...
                transport.on("sctpstatechange", (sctpState) => {
                    l('WebRtcTransport "sctpstatechange" event [sctpState:%s]', sctpState);
                });

                transport.on("dtlsstatechange", (dtlsState) => {
                    if (dtlsState === "failed" || dtlsState === "closed")
                        lw('WebRtcTransport "dtlsstatechange" event [dtlsState:%s]', dtlsState);
                });
            ...
            case "applyNetworkThrottle": {
                ...
                try {
                    ...
                    le("network throttle apply failed: %o", error);
            ...
            case "resetNetworkThrottle": {
                ...
                try {
                    ...
                    lw("network throttle stopped");

                    accept();
                } catch (error) {
                    le("network throttle stop failed: %o", error);
            ...
            default: {
                le('unknown request.method "%s"', request.method);
    ...
    async _createConsumer({ consumerPeer, producerPeer, producer }) {
        ...
        if (!transport) {
            lw("_createConsumer() | Transport for consuming not found");
        ...
        try {
            ...
            lw("_createConsumer() | transport.consume():%o", error);
        ...
        try {
            ...
            lw("_createConsumer() | failed:%o", error);
        }
    }
    ...
    async _createDataConsumer({
        ...
        // This should not happen.
        if (!transport) {
            lw("_createDataConsumer() | Transport for consuming not found");
        ...
        try {
            ...
            lw("_createDataConsumer() | transport.consumeData():%o", error);
        ...
        try {
            ...
            lw("_createDataConsumer() | failed:%o", error);
        }
    }
}
