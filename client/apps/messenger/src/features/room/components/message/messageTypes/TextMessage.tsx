...
export default function TextMessage({
    ...
    const showSeeMore =
        filteredText.length > maxTextCharsToShow ||
                    ...
                    >
                        {
                            <Box
                                ...
                                sx={{
                                    fontWeight: 400,
                                    fontStyle: "italic",
                                    color: isDark ? "#E5F4FF" : "primary.main",
                                    fontSize: "12px",
                                }}
                            >
...
function getTextToShow(
    ...
    maxTextCharactersToShow: number,
    maxTextLinesToShow: number,
) {
    if (showSeeMore && !isSeeMoreClicked) {
        if (text.split("\n").length > maxTextLinesToShow) {
            const lines = text.split("\n");
            return lines.slice(0, maxTextLinesToShow).join("\n");
        } else if (text.length > maxTextCharactersToShow) {
            return text.slice(0, maxTextCharactersToShow);
        }
    }
